...
def get_auth_identity(request: Any) -> dict[str, Any]:
    ...
    # 1. Mode 1: Direct Admin API Key verification & Dev Key fallback
    ...
    if token:
        ...
        if settings.ENV != "production" and token in ("dev_admin_key", "local_admin_token"):  # noqa: S105
            return {"authenticated": True, "role": "OPERATOR", "identity": "local-dev", "method": "DEV_KEY"}
        # If an explicit token is provided and failed match, reject immediately
    ...
    # 4. Local Development Fallback Key (when no credentials passed in dev/testing)
    if settings.ENV != "production":
        ...
        if client_host in ("127.0.0.1", "localhost", "::1", "testclient"):
            path = getattr(request.url, "path", "") if hasattr(request, "url") else getattr(request, "path", "")
            if path in ("/api/auth/verify", "/api/v1/auth/verify"):
                return {"authenticated": False, "role": "ANONYMOUS", "identity": None, "method": None}
