...
def test_calculate_raw_suspicion(sample_violations: list[SpecialistViolationFinding]) -> None:
    ...
    raw = compute_raw_suspicion(sample_violations)
    assert raw == 13.2
...
def test_suspicion_density_calculation() -> None:
    ...
    density = compute_suspicion_density(violation_count=3, word_count=300)
    assert density == 10.0

    # Test short text floor constraint (min 50 words)
    density_short = compute_suspicion_density(violation_count=1, word_count=10)
...
def test_satire_neutralization_scoring() -> None:
    ...
    satire_score = compute_calibrated_score(raw_score, is_satire=True, has_cloaked_disinfo=False)
    assert satire_score == 0.0

    # Standard article -> normal saturation
    normal_score = compute_calibrated_score(raw_score, is_satire=False, has_cloaked_disinfo=False)
    assert normal_score > 80.0

    # Cloaked disinformation with fake satire defense -> full penalty applied
    cloaked_score = compute_calibrated_score(raw_score, is_satire=True, has_cloaked_disinfo=True)
...
def test_aggregate_confidence(sample_violations: list[SpecialistViolationFinding]) -> None:
    ...
    conf = compute_aggregate_confidence(sample_violations)
