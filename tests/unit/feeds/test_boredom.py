...
from credence.models import FeedItem, FeedSubscription, TokenUsage, utc_now
...
async def test_boredom_cycle_digests_pending_items(db_session: AsyncSession):
    ...
    with (
        ...
        stmt_item = select(FeedItem).where(FeedItem.item_url == "https://wire.example.com/article-01")
...
async def test_boredom_cycle_respects_token_governor_budget(db_session: AsyncSession):
    ...
    sub = FeedSubscription(feed_url="https://sample.com/rss", title="Sample", priority_tier=2)
    db_session.add(sub)
    await db_session.commit()
    await db_session.refresh(sub)
