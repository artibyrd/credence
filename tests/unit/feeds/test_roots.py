...
async def test_expand_roots_registers_new_subscriptions(db_session: AsyncSession, tmp_path: Path):
    ...
    with (
        ...
        # Verify in database
        stmt_sub = select(FeedSubscription).where(FeedSubscription.feed_url == "https://courtwatch.org/feed.xml")
        sub = (await db_session.exec(stmt_sub)).first()
        assert sub is not None
        assert sub.title == "Court Watch Docket Feed"
        assert sub.is_active is True

        stmt_item = select(FeedItem).where(FeedItem.item_url == "https://courtwatch.org/cases/2026/brief-01")
