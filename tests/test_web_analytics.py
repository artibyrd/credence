...
Covers:
- Domain Epistemic Index (DEI) calculation & trust banding
...
from credence.models import AuditRecord, FeedSubscriptionRecord, SnapshotRecord, ViolationRecord
...
async def test_domain_epistemic_index_and_leaderboards(db_session: AsyncSession) -> None:
    ...
    assert honor_roll[0].dei_score >= 80.0
    ...
    assert wall_of_shame[0].dei_score < 40.0
...
def test_publisher_svg_badge_generation() -> None:
    ...
    svg = generate_publisher_svg_badge(domain="reuters.com", dei_score=98.5, status="HIGH_INTEGRITY", theme="dark")
