...
def test_dev_tfvars_syntax_and_types():
    ...
    dev_example = Path(__file__).resolve().parents[2] / "terraform" / "terraform.dev.tfvars.example"
...
def test_prod_tfvars_syntax_and_types():
    ...
    prod_example = Path(__file__).resolve().parents[2] / "terraform" / "terraform.prod.tfvars.example"
