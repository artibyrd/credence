...
from credence.db import get_async_session, get_engine, init_db
...
async def test_create_and_query_snapshot(db_session: AsyncSession) -> None:
    """Verify Snapshot can be inserted and queried asynchronously."""
    snapshot = Snapshot(
    ...
    # Query from database
    statement = select(Snapshot).where(Snapshot.url == "https://example.org/test-article")
...
async def test_create_audit_with_satire_flag(db_session: AsyncSession) -> None:
    """Verify Audit properly stores satire flags and Poe's law attributes."""
    snapshot = Snapshot(
...
async def test_violation_records_and_cascade_relationships(db_session: AsyncSession) -> None:
    """Verify child ViolationRecords are saved and linked to Audit."""
    snapshot = Snapshot(
    ...
    stmt = select(Violation).where(Violation.audit_id == audit.id)
