...
async def test_live_13_node_work_sharing_and_byzantine_defense(tmp_path: Path) -> None:
    ...
    try:
        ...
        # Verify aggregator detects ungrounded quote and excludes rogue node from 13-node cluster consensus
        ...
        assert verdict.classification == "LOW_SUSPICION", "Consensus should reject rogue ungrounded smear!"
        assert rogue_id.public_key_hex in verdict.outlier_nodes, "Rogue node should be identified as outlier!"
